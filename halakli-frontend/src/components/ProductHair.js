// src/components/ProductHair.jsx
...
const HAIR_SLUG =
  process.env.REACT_APP_CAT_HAIR_SLUG || "hair-products";
...
const API_ORIGIN =
  process.env.REACT_APP_PUBLIC_BASE_URL || "http://localhost:4000";
...
export default function ProductHair() {
  ...
  const [selectedForDiscount, setSelectedForDiscount] = useState(new Set());
  ...
  async function load(page = 1) {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/products/by-category?${qs.toString()}`
      );
  ...
  function scrollBulkList(direction) {
    ...
    const rowHeight = 40; // גובה שורה משוער בפיקסלים
    bulkListRef.current.scrollBy({
      top: direction * rowHeight * 3, // 3 שורות כל פעם
      behavior: "smooth",
    });
  }
  ...
  // 🌟 טעינת סטטיסטיקות צפיות/קניות לכל המוצרים ברשימה
  useEffect(() => {
    ...
    async function loadStats() {
      try {
        const url = `http://localhost:4000/api/pageviews/product-stats?${params.toString()}`;
        console.log("product-stats URL:", url);
        ...
        console.log("📊 productStats from API:", data, "=> map:", map);
        setProductStats(map);
      } catch (e) {
  ...
  async function saveEdit() {
    ...
    try {
      ...
      // ניקוי/נרמול רשימת התמונות
      images = images
        .map((im) => ({ url: (im.url || "").trim() }))
        .filter((im) => im.url.length > 0)
        .slice(0, 10);

      // 🔥 כאן אנחנו מגדירים מחדש את המערך – שלא יהיה no-undef
      const urlsToDelete = [];
      ...
      // 🟡 טיפול חכם במלאי – שונה בין יצירה לעריכה
      let payloadStockQty;
      if (editModal.isNew) {
        // במוצר חדש – אם לא מילאו, נחשב כ-0
        payloadStockQty =
          f.stock_qty !== "" && f.stock_qty != null
            ? Number(f.stock_qty)
            : 0;
      } else {
        // בעריכת מוצר – אם השדה ריק, לא נשלח בכלל stock_qty (לא נדרוס DB)
        payloadStockQty =
          f.stock_qty === "" || f.stock_qty == null
            ? undefined
            : Number(f.stock_qty);
      ...
      // רק אם יש ערך מפורש – נוסיף ל-payload
      ...
      if (editModal.isNew) {
        // למצוא קטגוריה לפי ה-slug של העמוד
        const cat = (categories || []).find(
          (c) =>
            c.slug === HAIR_SLUG ||
            c.category_slug === HAIR_SLUG
        );

        const createPayload = {
          ...payload,
          ...(cat?.id ? { category_id: cat.id } : {}),
          ...(cat?.id ? {} : { category_slug: HAIR_SLUG }),
        };
      ...
      // מחיקה פיזית של קבצים ישנים מהשרת
  ...
  return (
    <>
        ...
        <section className="acct-section">
          ...
          >
            <ol>
              ...
              <li aria-current="page">מוצרי שיער</li>
            </ol>
          </nav>

          <h1 className="acct-h1">מוצרי שיער</h1>
          <h2>קנו עכשיו את מוצרי השיער שלנו</h2>

          {/* 🔐 כפתורי אדמין – רק עם ?admin=1 */}
          {adminFromQuery && (
          ...
          {!loading && !err && items.length === 0 && (
            <p className="acct-sub">אין מוצרים בקטגוריה זו.</p>
          )}
              ...
              <div className="products-grid">
                {items.map((p) => {
                  ...
                  const mainSrc = normalizeImageUrl(
                    p.image_url ||
                      `https://picsum.photos/seed/hair${p.id}/600/600`
                  );
                  ...
                  const isLiked = liked.has(Number(p.id));

                  const stats = getStatsForProduct(
                    productStats,
                    p.id
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* כפתור מועדפים – רק למשתמשים רגילים (לא כש-adminFromQuery) */}
                      {!adminFromQuery && isAuthed && (
                        <button
                          type="button"
                          onClick={() =>
                            toggleFavorite(Number(p.id))
                          }
      ...
      {/* 🔐 מודאל הנחות מרוכזות – רק לאדמין וגם כשהוא פתוח */}
      {adminFromQuery && bulkDiscountModal.open && (
        <div
          ...
          <div className="cardError cardEdit">
            ...
            <div className="header">
              <div className="content" dir="rtl">
                ...
                <p className="message">
                  בחרי אילו מוצרים לקבל הנחה, סוג ההנחה, ולחצי
                  על &quot;החלת הנחות&quot;.
                </p>

                <div className="product-edit-grid">
                  {/* צד שמאל – רשימת מוצרים לבחירה */}
      ...
      {/* 🔐 חלון עריכה – רק לאדמין */}
      {adminFromQuery && editModal.open && (
        <div
          className="confirm-backdrop"
          role="dialog"
          aria-modal="true"
        >
          <div className="cardError cardEdit">
            ...
            <div className="header">
              ...
              <div className="content" dir="rtl">
                <span className="title">עריכת מוצר </span>
                <p className="message">
                  ערכי את פרטי המוצר ותמונות הגלריה.
                </p>
                ...
                <div className="product-edit-grid">
                  ...
                  <div className="product-edit-side">
                    <div className="product-edit-images-box">
                      ...
                      <div className="edit-field">
                        <small>
                          לחצי על תמונה כדי לבחור אותה
                          כתמונה הראשית, ניתן להעלות
                          תמונות חדשות.
                        </small>
                      </div>
