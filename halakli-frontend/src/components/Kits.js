...
// בסיס ציבורי – לשחזור URL מלא לקבצים
const PUBLIC_BASE =
...
// המקור לשרת (לסטטיסטיקות, תמונות וכו')
const API_ORIGIN =
  import.meta?.env?.VITE_PUBLIC_BASE_URL ||
  process.env.REACT_APP_PUBLIC_BASE_URL ||
  "http://localhost:4000";
...
export default function Kits() {
  ...
  // סטטיסטיקות צפיות/קניות לכל המוצרים ברשימה
  useEffect(() => {
    ...
    const ids = [...new Set(items.map((p) => Number(p.id)))].filter(
      Boolean
    );
    ...
    async function loadStats() {
      try {
        ...
        console.log("product-stats URL (KITS):", url);
    ...
    try {
      ...
      try {
        const imgsRes = await fetch(
          `${API_BASE}/api/products/${p.id}/images`
        );
        if (imgsRes.ok) {
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const secondaryCanonList = images.map((im) =>
        canonUrl(im.url)
      );

      const removedManual =
        Array.isArray(editModal.removedUrls) &&
        editModal.removedUrls.length
      ...
      if (editModal.isNew) {
        // למצוא קטגוריה לפי ה-slug של הערכות הביתיות
        const cat = (categories || []).find(
          (c) =>
            c.slug === HOME_KITS_SLUG ||
            c.category_slug === HOME_KITS_SLUG
        );

        const createPayload = {
          ...payload,
          ...(cat?.id ? { category_id: cat.id } : {}),
          ...(cat?.id ? {} : { category_slug: HOME_KITS_SLUG }),
        };
        ...
        <section className="acct-section">
          ...
          >
            <ol>
              ...
              <li aria-current="page">ערכות ביתיות</li>
            </ol>
          </nav>

          <h1 className="acct-h1">ערכות ביתיות</h1>
          <h2>קנו עכשיו את הערכות הביתיות שלנו</h2>
              ...
              <div className="products-grid">
                {items.map((p) => {
                  ...
                  const mainSrc = normalizeImageUrl(
                    p.image_url ||
                      `https://picsum.photos/seed/kits${p.id}/600/600`
                  );
        ...
        >
          <div className="cardError cardEdit">
            ...
            <div className="header">
              ...
              <div className="content" dir="rtl">
                <span className="title">עריכת ערכה ביתית</span>
                <p className="message">
